...
<div id="contentDiv">
    ...
    <div v-if="!loading && !dto.id" v-cloak>
        <%--<p style='text-align: center; line-height: 3em; vertical-align: middle;margin-top: 100px;'>无此账单信息...</p>--%>
        <div class="lyy_noorder">暂无账单信息</div>
    </div>
    <div class="lyy_nonav_box" style="bottom: 75px;" v-if="!loading && dto.id">
    ...
    <div class="lyy_footer container" v-if="!loading && dto.id">
    ...
    var vm = new Vue({
        ...
        methods: {
            getBillsInfo : function() {
                var _this = this;
                var id = $("#id").val();
                var url = contextPath + "/accounts/getBillById?id=" + id;
            ...
            save : function() {
                myConfirm("确定要保存？",function(){
                    ...
                    var url = contextPath + "/accounts/updateAccounts";
                    axios.post(url,vm.dto).then(function(response) {
                        ...
                        if (obj.status == "0000") {
                            ...
                            myAlert("修改失败，已存在" + vm.dto.addDate + "当天" + vm.convertFlag(vm.dto.flag) + "的记录!");
                        } else {
                            myAlert(obj.message ? obj.message : "修改失败！");
                        }
                        dialog.close();
                    });
                });
